...
class Auto extends Thread { // Class for the auto threads.
	...
	public void run() 
	   ...
	   while (true) 
		...
		if (port == Sem.PORT0){
			semBoard = fry.semBoardPort0;
		}
		else {
			semBoard = fry.semBoardPort1;
		}
		// Board
		try {
			semBoard.acquire();
		...
		fry.addLoad(port);  // increment the ferry load
		
		if (fry.getLoad() == Sem.MAXLOAD){
		...
		fry.reduceLoad(port);   // Reduce load
		
		if (fry.getLoad() == 0)
			semBoard.release(); // signal to cars to board
		else
			fry.semDisembark.release(); // signal next auto to disembark
...
class Ambulance extends Thread { // the Class for the Ambulance thread
	...
	public void run() 
	   ...
	   while (true) 
		...
		fry.addLoad(port);  // increment the load 
		...
		if(fry.getLoad() == 0){ 
			...
			fry.semDisembark.release();  // signal car to disembark
		}
...
class Ferry extends Thread // The ferry Class
{
	...
	public void reduceLoad(int port) {
